...
t = p.read_text()
t = re.sub(r'_?CUDA_MAXIMUM_COMPUTE_CAPABILITY\s*=\s*\([0-9]+,\s*[0-9]+\)',
           '_CUDA_MAXIMUM_COMPUTE_CAPABILITY = (10, 3)', t)
p.write_text(t)
...
_USE_FLASH_ATTENTION_3 = _fa3_supported()
'''.strip()
t = re.sub(r'_?USE_FLASH_ATTENTION_3\s*=\s*True', _runtime_check, t)
p.write_text(t)
